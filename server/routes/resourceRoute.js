...
router.put("/review/:id/approve", async (req, res) => {
  try {
    ...
    if (review.action_type === "add") {
      ...
      if (review.id) {
        //Update the existing pending resource to "Approved" status
        const pendingResource = await Resource.findOne({ id: review.id, status: "Pending" });

        if (!pendingResource) {
          console.warn(`No pending resource found with id: ${review.id}`);
          return res.status(404).json({ message: `Pending resource with ID ${review.id} not found` });
        }
        ...
        //No pending resource record, creation of a new one
        const newId = await generateId();
        const newResource = new Resource({
          id: newId,
    ...
    } else if (review.action_type === "edit") {
      console.log(`Processing EDIT action for review ID: ${req.params.id}, resource ID: ${review.original_id}`);
      ...
      if (!updatedResource) {
        console.warn(`No resource found with id: ${review.original_id}`);
        return res.status(404).json({ message: `Resource with ID ${review.original_id} not found` });
      }

      console.log(`Resource updated successfully, ID: ${updatedResource.id}`);
    } else if (review.action_type === "delete") {
      console.log(`Processing DELETE action for review ID: ${req.params.id}, resource ID: ${review.original_id}`);

      // Delete the resource
      const deletedResource = await Resource.findOneAndDelete({ id: review.original_id });

      if (!deletedResource) {
        console.warn(`No resource found to delete with id: ${review.original_id}`);
        return res.status(404).json({ message: `Resource with ID ${review.original_id} not found` });
      }

      console.log(`Resource deleted successfully, ID: ${review.original_id}`);
    }
