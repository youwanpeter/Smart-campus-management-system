  ...
  const handleCreateOrUpdate = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const refreshResponse = await fetch('http://localhost:5000/api/subjects');
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (level) => {
        ...
        return <Tag color={color}>{level.charAt(0).toUpperCase() + level.slice(1)}</Tag>;
      },
    },
    {
      title: 'Instructors',
      ...
      >
        <Form form={form} layout="vertical" name="subject_form">
          ...
          <Form.Item
            ...
            label="Subject Level"
            rules={[
              { required: true, message: "Please input subject level." },
            ]}
          >
            <Select>
